...
mrcas <- c("HGC-27_MRCA_harvest", "MDA-MB-231_MRCA_harvest", "SNU-668_MRCA_harvest", "SUM-159_NLS_2N_MRCA_harvest", "SUM-159_NLS_4N_MRCA_harvest" )
...
## Exclude marker chromosome:
karyotypes$karyotype = sapply(karyotypes$karyotype, function(x) x[1:22], simplify = F)
## Exclude pop-average resolution @TODO: should happen upstream
keep = sapply(karyotypes$karyotype, function(x) !any(x -round(x) != 0))
karyotypes <- tibble::tibble(id = karyotypes$id[which(keep)], karyotype = karyotypes$karyotype[which(keep)])
```
...
fit_objects <- lapply(names(grouped_map), function(cond_name) {
  ...
  print(names(KT_list))
  print("**")
  print(sapply(group, function(x) x$id_end))
